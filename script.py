...
def blrObjFunction(initialWeights, *args):
    ...
    Input:
        ...
        labeli: the label vector (y_k) of size N x 10 where each entry can be either 0 or 1 representing the label of corresponding feature vector
...
def mlrObjFunction(initialWeights_b, *args):
    ...
    train_data = np.concatenate((np.ones((n_data, 1)), train_data), axis=1)  #N*(D+1)
    sumexp = np.sum(np.exp(np.dot(train_data,initialWeights_b)), axis=1)
    theta = np.exp(np.dot(train_data,initialWeights_b)) / sumexp  #N*10
    error = -np.sum(labeli * np.log(theta))
    error_grad = np.dot(np.transpose(train_data),(theta - labeli))
...
def mlrPredict(W, data):
    ...
    data = np.concatenate((np.ones((data.shape[0], 1)), data), axis=1)
    sumexp = np.sum(np.exp(np.dot(train_data, initialWeights_b)), axis=1)
    theta = np.exp(np.dot(train_data, initialWeights_b)) / sumexp
    label = np.argmax(theta,axis=1)
...
SVM_train_data = train_data[random_points]
SVM_train_label = train_label[random_points]
#
# # Selecting random 2000 points
# random_points = np.random.choice(10000, 2000)
# SVM_validation_data = validation_data[random_points]
# SVM_validation_label = validation_label[random_points]
# SVM_test_data = test_data[random_points]
# SVM_test_label = test_label[random_points]

# Linear Kernel
clf = SVC(kernel='linear')
clf.fit(SVM_train_data, SVM_train_label.flatten())
print('\n Fitting for Linear Kernel Completed')
print('\n Training set Accuracy:', clf.score(SVM_train_data, SVM_train_label))
...
for i in range(len(c_values)):
    ...
    clf.fit(SVM_train_data, SVM_train_label.flatten())
    train_accuracy[i] = 100*clf.score(SVM_train_data, SVM_train_label)
